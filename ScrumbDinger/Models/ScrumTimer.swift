...
final class ScrumTimer: ObservableObject {
    ...
    private(set) var lengthInMinites: Int
    ...
    private var lengthInSeconds: Int { lengthInMinites * 60 }
    private var secondsPerSpeaker: Int {
        (lengthInMinites * 60) / speakers.count
    }
    ...
    func startScrum() {
        // [weak self] prevent program from memory leaking.
        timer = Timer.scheduledTimer(withTimeInterval: frequency, repeats: true, block: { [weak self] _ in
            self?.update()
        })
        timer?.tolerance = 0.1
    ...
    private func changeToSpeaker(at index: Int) {
        ...
        // note the startDate is not the meeting's startDate
        startDate = Date()
    }
    ...
    private nonisolated func update() {
        Task { @MainActor in
            ...
            // why guard here: note the <= and >=, only when secondsElapsed = secondsPerSpeaker will trigger the if statement below
